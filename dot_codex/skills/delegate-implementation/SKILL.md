...
Grok is the default implementer for this kind of work. Keep taste-sensitive work — public APIs, UI, copy — in the current session.
...
Summarize what actually changed, call out anything risky, and run the acceptance check in the main checkout after merge. If the delegate's changes miss the bar, redo the work in the current session rather than polishing its output.
